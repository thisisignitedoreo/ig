...
int main(void) {
	...
	while (!WindowShouldClose()) {
		BeginDrawing();
			...
			ig_begin_window("cool", IG_RESIZABLE | IG_MOVABLE | IG_TOPBAR, (Rectangle) {GetScreenWidth()/2-100, 10, 200, 100});
			ig_end_window();

			ig_begin_window("fu fu", IG_RESIZABLE | IG_MOVABLE | IG_TOPBAR, (Rectangle) {GetScreenWidth()/2-100, 10, 200, 100});
			ig_end_window();
